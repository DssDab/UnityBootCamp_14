...
public class VectorSample : MonoBehaviour
{
    ...
    // 낮은 주소 : 메모리의 시작 부분 (가벼운 데이터 부터 들어감)
    // 높은 주소 : 메모리의 끝 부분  (무거운 데이터가 들어감)
    ...
    // 2. 데이터(data) : 프로그램에서 전역변수, 정적 변수가 저장되는 영역
    //                  전역 변수(global) : 프로그램 어디서나 접근 가능한 변수(C#에서는 전역 대신
