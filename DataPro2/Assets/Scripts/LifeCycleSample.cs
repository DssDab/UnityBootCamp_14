...
public class LifeCycleSample : MonoBehaviour
{
    ...
    {
       Debug.Log($"프레임 수 : {count_per_frame} [FixedUpdate] 물리에 대한 업데이트가 진행되는 영역");
    }

    void Update()
    {
        count_per_frame++;  // 카운트 증가
        Debug.Log($"프레임 수 : {count_per_frame} [Update] 게임 로직에 대한 호출이 진행되는 영역");

        if (count_per_frame == 3)
        {
            Debug.Log($"프레임 수 : {count_per_frame} [Test 1] 오브젝트의 비활성화 작업을 진행합니다.");
            gameObject.SetActive(false);
        }

        if (count_per_frame == 6)
        {
            Debug.Log($"프레임 수 : {count_per_frame} [Test 2] 오브젝트의 활성화 작업을 진행합니다.");
            gameObject.SetActive(true);
        }
        if (count_per_frame == 9)
        {
            Debug.Log($"프레임 수 : {count_per_frame} [Test 3] 오브젝트의 파괴 작업을 진행합니다.");
            Destroy(gameObject);
        }
    }
    private void LateUpdate()
    {
        Debug.Log($"프레임 수 : {count_per_frame} [LateUpdate] 물리 연산, 로직 이후의 후 처리");
    }
    ...
    IEnumerator CustomCoroutine()
    {
        ...
        yield return new WaitForSeconds(1f);        // 1초에 대한 대기
        Debug.Log("[Coroutine] 1초 후 다시 실행됩니다.");

        yield return new WaitForFixedUpdate();
        Debug.Log("[Coroutine] FixedUpdate 가 지나면 다시 실행됩니다.");
        
        yield return new WaitForEndOfFrame();
        Debug.Log("[Coroutine] 프레임의 끝이 지나면 다시 실행됩니다.");
    ...
    private void OnDestroy()
    {
        ...
        // gameObject.SetActive(true / false) --> 진행이 되도 실질적으로 의미가 없음
