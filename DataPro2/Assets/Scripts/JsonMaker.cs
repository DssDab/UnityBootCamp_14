...
public class JsonMaker : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        QuestList questList = new QuestList()
        {
            ...
            {
                // new 생성자명() {필드명1 = 값, 필드명2 = 값 ...} 해당 형태의 값을 가진 클래스 객체가 생성됩니다.

                new QuestData() {questName = "시작이 반이다.", reward = "EXP + 100", description="시작이라도 하면 반이라도 가지."},

                new QuestData() {questName = "중간만 해라.", reward ="EXP + 150", description = "중간이라도 하는게 어디야"},

                new QuestData() {questName = "할거면 끝까지 해라.", reward ="EXP + 200", description = "그냥 다 하라는거잖아.."}
        ...
        // C#교과서 720 page : System.IO 네임스페이스
        //         755 page : Path 클래스를 통해 파일 이름, 확장자, 폴더 정보 얻는 방법
