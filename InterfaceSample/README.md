...
- ✅ **Distance**: 두 벡터 간의 거리 계산  
  → `Vector3.Distance(a, b)`는 위치 간의 유클리디안 거리 반환

- ✅ **Lerp (선형 보간)**: 두 벡터 사이를 지정된 비율로 보간  
  → `Vector3.Lerp(start, end, t)`에서 `t`는 0~1 사이의 비율

- ✅ **Slerp (구면 선형 보간)**: 곡선을 따라 더 부드럽게 보간  
  → `Vector3.Slerp(start, end, t)`로 방향 벡터를 자연스럽게 전환할 때 유용

- ✅ **Mathf.Sqrt()**: 벡터 계산에 자주 사용되는 제곱근 함수  
  → 거리 계산: `Mathf.Sqrt(x * x + y * y + z * z)`
