...
// 네임 스페이스 사용방법
// 1. namespace.클래스명과 같이 네임스페이스를 직접 작성해서 사용하는 경우

// 2. using을 이용해 해당 코드 내에서 사용하는 값은 그 네임스페이스의 값임을 알리고 사용하는 경우

//2. 클래스의 이름은 BasicScripts
//   이 스크립트는 MonoBehaviour의 기능을 포함하고 있습니다. ( A : B 형태 == A가 B를 상속했다)
// 스크립트의 이름과 같은 Class가 반드시 존재해야 합니다.

// Monobehaviour 클래스는 에디터에서 게임 오브젝트에 스크립트를 커포넌트로써 연결할 수 있는 프레임워크를 제공합니다.
// Start, Update와 같은 이벤트에 대한 연결도 제공합니다.
public class BasicScripts : MonoBehaviour
{

    // 3. 유니티 생명 주기(이벤트)
    // 유니티에서는 스크립트를 실행할 경우 사전에 지정된 순서대로 여러 개의 이벤트 함수가 실행이 됩니다.
