...
public class EventSample : MonoBehaviour
{
    ...
    // 이벤트 변수의 이름은 보통 On + 동사 / 시제로 번들어집니다.
    ...
    // EventArgs e <- 이벤트와 관련된 데이터를 담고있는 객체를 의미합니다.
    ...
    void Update()
    {
        // 1) 이벤트 실행 박식 이벤트명(this, EventAgrs.Empty)
        if (Input.GetKeyDown(KeyCode.Space))    // 스페이스 버튼 클릭
        {
            // Null 검사를 진행하고 실행(이벤트의 구독이 안되었을 경우에는 실행하면 안되기 때문)
    ...
    {
        Debug.Log("<color=yellow>엔터키 입력 이벤트 실행</color>");
    }
